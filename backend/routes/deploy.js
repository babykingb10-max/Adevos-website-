...
router.post('/deploy', userAuth, async (req, res) => {
  try {
    ...
    // and can be polled via GET /api/deploy/logs/:id or streamed via SSE.
    (async () => {
      try {
...
router.get('/deploy/logs/:id', userAuth, async (req, res) => {
...
router.get('/deploy/stream/:id', userAuth, async (req, res) => {
  res.writeHead(200, {
...
// ---------- LIST MY DEPLOYMENTS ----------
router.get('/deploy/mine', userAuth, async (req, res) => {
  const deployments = await Deployment.find({ user: req.user._id }).populate('bot platform');
  res.json({ success: true, data: deployments });
});

// ---------- MANAGE: STOP / RESTART ----------
router.post('/deploy/:id/stop', userAuth, async (req, res) => {
...
router.post('/deploy/:id/restart', userAuth, async (req, res) => {
...
router.put('/deploy/:id', userAuth, async (req, res) => {
  const deployment = await Deployment.findById(req.params.id);
  if (!deployment || String(deployment.user) !== String(req.user._id)) {
    return res.status(404).json({ success: false, message: 'Deployment not found' });
  }
...
// ---------- DELETE DEPLOYMENT ----------
router.delete('/deploy/:id', userAuth, async (req, res) => {
  const deployment = await Deployment.findById(req.params.id).populate('platform');
  if (!deployment || String(deployment.user) !== String(req.user._id)) {
    return res.status(404).json({ success: false, message: 'Deployment not found' });
  }
